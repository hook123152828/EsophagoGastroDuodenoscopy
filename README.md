...
**分割是掃描的關鍵路徑**（14 分鐘的影片上約佔八成），而且**不能靠併發加速**：
`gim_server` 的 `/predict` 是同步 `def`，第二個併發請求會讓第二條執行緒進入
同一個 mmseg 模型，導致靜默卡死（服務連自己的 `/health` 都不再回應）。
要加速只能**多開行程**，用 `GIM_REPLICAS` 設定（預設 2，每個約佔 5 GB 顯存），
gateway 端 round-robin，且保證同一個行程同時只有一個請求。

實測 video1.mp4（50,455 幀，60/60/60）：

| GIM 行程數 | 掃描時間 | GPU 使用率 |
|---|---|---|
| 1 | 546 s | 33–37% |
| 2 | 464 s（1.18×） | 62–65% |

> 加速幅度遠低於行程數的比例。GIM 變快之後瓶頸就轉移到與 GNS 的顯卡爭用——
> 兩個行程的理論下限是 217 秒，實測 464 秒，中間的差距全是爭用。
> 再往上加行程的效益會更小。
...
**上傳影片失敗**
只接受 `.mp4 .avi .mov .mkv`，其餘副檔名會回 400。

錯誤訊息若是 **413 且提到 nginx**，那不是本系統回的——這個專案沒有 nginx。
是中間的反向代理（例如 VS Code dev tunnel）在邊緣就擋掉了請求，gateway 從未收到。
前端的上傳是分塊送的，遇到 413 會自動把塊大小減半重試（8 MB 起，下限 256 KB），
所以正常情況下會自己適應。若連 256 KB 都被拒，最快的替代方案是**直接把影片檔
放進 `video/`**——選片清單掃的就是這個目錄，完全不經過 HTTP 上傳。

上傳中斷可以續傳：重新選同一個檔案就會從斷點接續。未完成的暫存檔在
`video/.uploads/`，目前不會自動清理。
